...
def _build_monitor_ui(container):
    ...
    style.configure("HiddenTabs.TNotebook", background=UI_THEME["bg"], borderwidth=0, relief="flat", highlightthickness=0)
    ...
    top_separator = tk.Frame(container, bg=UI_THEME.get("border", "#3C3C3C"), height=1)
    ...
    def _refresh_theme_in_place():
        ...
        try:
            ...
            style_local.layout("HiddenTabs.TNotebook.Tab", [])
            style_local.configure("HiddenTabs.TNotebook", background=UI_THEME["bg"], borderwidth=0, relief="flat", highlightthickness=0)
        ...
        try:
            ...
            notebook_shadow.configure(bg="#000000")
        except Exception:
            pass
    ...
    def _apply_density(_mode_label=None):
        ...
        try:
            ...
            tabs_shell.pack_configure(pady=(gap, theme_space("space_3", 10)))
        except Exception:
            pass
    ...
    tabs_shell = tk.Frame(container, bg=UI_THEME["bg"])
    tabs_shell.pack(fill=tk.BOTH, padx=theme_space("space_3", 10), pady=(theme_space("space_2", 8), theme_space("space_3", 10)), expand=True)

    tabs_bar = tk.Frame(tabs_shell, bg=UI_THEME["bg"])
    tabs_bar.pack(fill=tk.X, pady=(0, 0))

    notebook_shadow = tk.Frame(tabs_shell, bg="#000000", height=1)
    notebook_shadow.pack(fill=tk.X, padx=0, pady=(0, 0))

    notebook = ttk.Notebook(tabs_shell, style="HiddenTabs.TNotebook")
    ...
    def _refresh_tab_buttons():
        ...
        for btn, tab_id in tab_buttons:
            selected_tab = str(selected) == str(tab_id)
            try:
                btn.configure(
                    bg=UI_THEME.get("surface", UI_THEME["bg"]) if selected_tab else UI_THEME.get("surface_alt", UI_THEME["surface"]),
                    fg=UI_THEME.get("on_primary", UI_THEME["text"]) if selected_tab else UI_THEME.get("on_surface", UI_THEME["text"]),
                    highlightbackground=UI_THEME.get("border", "#3C3C3C"),
                    activebackground=UI_THEME.get("border", UI_THEME.get("surface_alt", UI_THEME["surface"])),
                )
            except Exception:
                continue

    def _select_tab(tab_index: int):
        try:
    ...
    for idx, label in enumerate(("CONTROLE", "ENCOMENDAS", "ORIENTAÇÕES", "OBSERVAÇÕES")):
        btn_tab = build_secondary_button(tabs_bar, label, lambda i=idx: _select_tab(i))
        btn_tab.pack(side=tk.LEFT, padx=(0 if idx == 0 else 6, 0), pady=(0, 0))
        tab_buttons.append((btn_tab, notebook.tabs()[idx]))

    notebook.bind("<<NotebookTabChanged>>", lambda _e: _refresh_tab_buttons(), add="+")
    _refresh_tab_buttons()
