...
def _build_monitor_ui(container):
    ...
    for idx, key in enumerate(["ativos", "pendentes", "sem_contato", "avisado"]):
        ...
        try:
            ...
            card.set_donut_mode("hybrid" if key in {"pendentes", "sem_contato"} else "hidden")
        except Exception:
            pass
    ...
    def _on_card_variant_change(_event=None):
        ...
        tones = {"ativos": ("Ativos", "info", "📦"), "pendentes": ("Pendentes", "warning", "⏳"), "sem_contato": ("Sem contato", "danger", "☎"), "avisado": ("Avisado", "success", "✅")}
        cards_widgets[:] = []
        for idx, key in enumerate(order):
            title, tone, icon = tones[key]
            card = AppMetricCard(cards_row, title, tone=tone, icon=icon, variant=selected_variant)
            card.grid(row=0, column=idx, padx=(0, theme_space("space_2", 8)), sticky="ew")
            card.set_action(lambda k=key: _apply_card_drilldown(k))
            card.set_donut_mode("hybrid" if key in {"pendentes", "sem_contato"} else "hidden")
            _ux_cards[key] = card
