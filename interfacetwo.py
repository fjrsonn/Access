...
def _build_text_actions(frame, text_widget, info_label, path):
    ...
    def apply_status(status):
        ...
        if not rec:
            return
        if not is_encomendas:
            try:
                source = _monitor_sources.get(text_widget, {})
                filter_key = source.get("filter_key", text_widget)
                current_filters = dict(_filter_state.get(filter_key) or _default_filters())
                current_filters["status"] = "Todos"
                _filter_state[filter_key] = current_filters
            except Exception:
                pass
        try:
